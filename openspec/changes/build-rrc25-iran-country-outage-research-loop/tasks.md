...
- [x] 0.6 发布 create-only DB-first JSON、中文摘要与 `SHA256SUMS`
...
- [x] 4.3 用数据库影响索引定位候选，再以原始映射复核代表 IR 前缀/origin
- [ ] 4.4 实现 record 边界检查点、不可变 JSONL gzip 分片、原子发布和哈希绑定恢复
- [ ] 4.5 验证中断、重跑、不同并发顺序和检查点失配均不会改变语义结果或覆盖旧制品

## 5. 国家样本与影响人口

- [x] 5.1 从数据库冻结兼容 cohort、六小时中位数基线与实际窗口
- [x] 5.2 发布 1,928 槽国家指标与 `value_state/missing_reason`，禁止未知补零
- [x] 5.3 分开解释数据库 IPv4 `/24`、IPv4 等效地址和 IPv6 `/48`，不冒充地址并集
- [x] 5.4 从事实活动区间派生关键时刻 ASN/前缀集合、历史峰值分类和恢复候选
- [x] 5.5 仅将代表 ASN/前缀变化关联到 RouteEvent/raw refs，并明确样本不可外推
