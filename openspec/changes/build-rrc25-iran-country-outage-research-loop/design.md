...
才负责资源估算、关键槽解析、断点恢复、质量门和最终组装。每个 raw worker 在九分钟
软限前于 record 边界退出；累计用量由运行 manifest 汇总。任何数据库写目标、5GB
临时空间风险或 50GB 新增原始读取风险在执行前拒绝。
...
5. 仅为确认过的字段缺口生成 evidence sidecar；质量门失败时保留失败证据。
6. 完成伊朗事件对账和中文报告；不激活生产、不写数据库。
