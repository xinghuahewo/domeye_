...
class EvidenceGraphManager:
    ...
    def commit(
        ...
        *,
        ...
        node_type_by_plan: dict[str, str] = {}
        for node_id in [node["node_id"] for node in plan["nodes"] if node["node_id"] in records]:
            ...
            node_type_by_plan[node_id] = node_type
        ...
        for node_id, record in records.items():
            if not str(record["unit_id"]).startswith("OP-"):
                continue
            # 失败/取消/依赖跳过的 Operator 仍以 execution_failure 节点保留，
            # 但它没有发布确定性事实，因而不得伪造 derived_from 事实边。
            if node_type_by_plan.get(node_id) != "derived_fact":
                continue
