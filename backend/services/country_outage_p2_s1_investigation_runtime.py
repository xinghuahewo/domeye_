...
def build_local_fixture_sidecar_runtime(
    repository_root_path: str | Path,
    store_root: str | Path,
    *,
    event_reference: str,
    ...
    sidecar_shared_token: str,
    answer_fixture_id: str,
    answer_fixture_digest: str,
) -> "CountryOutageP2S1InvestigationRuntime":
    """正式本地 factory：同一 runtime 同时装配 Host execution 与 loopback Answer Port。"""

    runtime = build_local_fixture_runtime(
        repository_root_path, store_root,
        event_reference=event_reference,
        planning_grounding_port=planning_grounding_port,
        model_port=None,
    )
    identity = runtime._identity(event_reference, publication_id, publication_revision)
    runtime.model_port = LocalFixtureSidecarModelPort(
        base_url=sidecar_base_url,
        shared_token=sidecar_shared_token,
        fixture_id=answer_fixture_id,
        expected_fixture_digest=answer_fixture_digest,
...
class LocalFixtureSidecarModelPort:
    """loopback-only W5 fixture Sidecar adapter；消费 Host 冻结回答上下文。"""
    ...
    def create_turn(self, request: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        if not isinstance(request_identity, Mapping) or any(request_identity.get(field) != self.expected_identity.get(field) for field in identity_fields):
            raise W5InvestigationError("model_sidecar_identity_mismatch", "Host request 未绑定配置的 investigation 身份", status_code=502)
        ready = self._json("/readyz")
        if ready.get("ready") is not True or ready.get("execution_mode") != "trusted_fixture_replay_only" or ready.get("external_provider_enabled") is not False or ready.get("runtime_integrated") is not True or ready.get("production_deployed") is not False:
        ...
        result = self._json("/answer-turns", {
            "schema_version": "country_outage_p2_s1_w5_integrated_answer_request_v1",
            "fixture_id": self.fixture_id,
            "fixture_digest": self.expected_fixture_digest,
            "idempotency_key": key,
            "request": copy.deepcopy(dict(request)),
        })
        if result.get("schema_version") != "country_outage_p2_s1_w5_model_turn_v1" or result.get("fixture_id") != self.fixture_id or result.get("fixture_digest") != self.expected_fixture_digest:
            raise W5InvestigationError("model_sidecar_binding_mismatch", "W5 Sidecar fixture 身份漂移", status_code=502)
        binding = result.get("shared_answer_binding")
        if not isinstance(binding, Mapping) or binding != request.get("shared_answer_binding"):
            raise W5InvestigationError("model_sidecar_identity_mismatch", "Sol→Host→DS 流未绑定同一 investigation 身份", status_code=502)
        if result.get("shared_answer_binding_digest") != request.get("shared_answer_binding_digest"):
            raise W5InvestigationError("model_sidecar_binding_mismatch", "Sidecar shared binding digest 漂移", status_code=502)
...
_PROHIBITED_ANSWER_CLAIM = re.compile(
    r"customer\s*cone|客户锥|客户关系|商业关系|因果|原因是|导致|全国(?:断网|中断|影响)|"
    r"用户影响|业务影响|责任|已经恢复|恢复完成|recovered|caused\s+by|nationwide|user\s+impact",
    re.IGNORECASE,
)


def _host_answer_oracle(identity: Mapping[str, Any]) -> dict[str, Any]:
    boundary_texts = [
        "仅表达绑定 publication 下 RRC25 BGP 控制面观测。",
        "不将控制面观测解释为客户锥、商业关系、全国或用户与业务影响。",
        "不声称事件因果、责任或恢复完成。",
    ]
    boundary_assertions = [f"oracle:Q01:boundary:{hashlib.sha256(canonical_json(text).encode('utf-8')).hexdigest()[:24]}" for text in boundary_texts]
    limitations = [
        "仅覆盖绑定 publication、RRC25 collector 与已提交调查图。",
        "离线 fixture replay 未调用外部 Sol/DS provider，不代表生产模型效果。",
        "控制面证据不能证明客户锥、商业关系、全国或用户与业务影响、因果、责任或恢复完成。",
    ]
    unknown_texts = [
        "真实原因与责任未知。",
        "全国、用户及业务影响未知。",
    ...
    unknowns = [f"oracle:Q01:unknown:{hashlib.sha256(canonical_json(text).encode('utf-8')).hexdigest()[:24]}" for text in unknown_texts]
    base = {
        "schema_version": "country_outage_p2_s1_w5_host_answer_oracle_v1",
        "boundary_texts": boundary_texts,
        "boundary_assertions": boundary_assertions,
        "limitations": limitations,
        "unknown_texts": unknown_texts,
        "unknowns": unknowns,
        "prohibited_claim_patterns": ["customer_cone", "commercial_relationship", "causality", "nationwide_or_user_impact", "responsibility", "recovery"],
    }
    return {**base, "oracle_digest": digest_prefixed(base)}


def _host_graph_facts(graph: Mapping[str, Any]) -> list[dict[str, Any]]:
    facts = []
    for node in graph.get("nodes", []):
        if not isinstance(node, Mapping) or node.get("node_type") not in {"result_set", "derived_fact"}:
            continue
        payload = node.get("payload")
        if not isinstance(payload, Mapping):
            raise W5InvestigationError("turn_graph_payload_missing", "回答所用 Graph fact 缺少受信 payload")
        evidence_refs = sorted(str(item) for item in node.get("evidence_refs", []) if isinstance(item, str) and item)
        if not evidence_refs:
            raise W5InvestigationError("turn_graph_evidence_missing", "回答所用 Graph fact 缺少 Evidence")
        fact_id = payload.get("fact_id") if node["node_type"] == "derived_fact" else None
        if not isinstance(fact_id, str):
            fact_id = "fact-sha256:" + digest_hex({"graph_node_id": node["node_id"], "payload_digest": node["payload_digest"]})
        claim_kind = "observed_fact" if node["node_type"] == "result_set" else "derived_fact"
        claim_relation = "states_observed_fact" if claim_kind == "observed_fact" else "states_derived_fact"
        fact_label = "观测事实" if claim_kind == "observed_fact" else "确定性派生事实"
        source_value_digest = str(node["payload_digest"])
        allowed_claim_text = (
            f"RRC25 当前绑定调查图已提交{fact_label} {fact_id}"
            f"（结构化值摘要 {source_value_digest}）。"
        )
        facts.append({
            "fact_id": fact_id,
    ...
    if not facts:
        raise W5InvestigationError("turn_graph_fact_missing", "回答必须绑定至少一个已提交 Graph fact")
    return facts
...
class CountryOutageP2S1InvestigationRuntime:
    ...
    def _view(self, value: Mapping[str, Any], current_digest: str) -> dict[str, Any]:
        ...
        turn_refs = [{
            "turn_id": ref["turn_id"],
            "turn_revision": ref["turn_revision"],
            "answer_id": ref["answer_id"],
            "answer_revision": ref["answer_revision"],
        } for ref in value.get("turn_refs", [])]
        return {
    ...
    def create_turn(self, principal: Mapping[str, Any], investigation_id: str, body: Mapping[str, Any]):
        _, value, pointer = self._owned(principal, investigation_id)
        prior = self.store.read_idempotency("investigation", investigation_id, str(body.get("idempotency_key", "")))
        if prior is not None:
            if prior["request_digest"] != digest_prefixed(body):
                raise W5InvestigationError("idempotency_conflict", "同一幂等 key 对应不同追问")
            committed = self.store.get_json("investigation", prior["outcome"]["object_digest"])
            ref = next((item for item in committed.get("turn_refs", []) if item.get("idempotency_key_digest") == digest_prefixed({"idempotency_key": body["idempotency_key"]})), None)
            if ref is None:
                raise W5InvestigationError("turn_idempotency_outcome_invalid", "追问幂等 outcome 缺少 turn ref")
            turn = self._turn_from_ref(committed, ref)
            return {"accepted": True, "deduplicated": True, "turn": self._turn_view(turn), "investigation": self._view(committed, prior["outcome"]["object_digest"])}, 202
        revision, expected, key = self._cas(body, pointer)
        ...
        if value.get("evidence_graph_revision") is None or not value.get("evidence_graph_refs"):
            raise W5InvestigationError("turn_evidence_graph_missing", "追问必须绑定已提交 Evidence Graph")
        graph_ref_internal = value["evidence_graph_refs"][-1]
        graph = self.store.get_json("evidence-graph", graph_ref_internal["object_digest"])
        if graph.get("graph_revision") != value["evidence_graph_revision"]:
            raise W5InvestigationError("turn_evidence_graph_stale", "current Evidence Graph revision 漂移")
        result_refs = []
        for ref in value.get("result_set_refs", []):
        ...
        result_refs.sort(key=lambda item: (item["result_set_id"], item["result_set_revision"]))
        evidence_refs = sorted({
            str(evidence_ref)
            for graph_node in graph.get("nodes", [])
            if isinstance(graph_node, Mapping)
            for evidence_ref in graph_node.get("evidence_refs", [])
            if isinstance(evidence_ref, str) and evidence_ref
        })
        plan_ref = {
            "plan_id": value["plan"]["plan_id"],
            "plan_revision": value["plan"]["plan_revision"],
        ...
        graph_facts = _host_graph_facts(graph)
        host_oracle = _host_answer_oracle(value["identity"])
        binding = {
            "investigation_id": investigation_id,
            "source_investigation_revision": revision,
            "source_current_digest": expected,
            "identity_digest": value["identity"]["identity_digest"],
            "plan_ref": plan_ref,
            "result_set_refs": result_refs,
            "evidence_graph_ref": graph_ref,
            "host_oracle_digest": host_oracle["oracle_digest"],
            "question_digest": digest_prefixed(body["question"]),
            "anchor": copy.deepcopy(dict(anchor)),
            "registry_snapshot_id": value["identity"]["registry_snapshot_id"],
            "registry_snapshot_digest": value["identity"]["registry_snapshot_digest"],
            "binding_generation": value["identity"]["binding_generation"],
        }
        binding_digest = digest_prefixed(binding)
        request = {
            "schema_version": "country_outage_p2_s1_w5_model_turn_request_v1",
            "investigation_id": investigation_id,
            "source_investigation_revision": revision,
            "source_current_digest": expected,
            "identity": copy.deepcopy(value["identity"]),
        ...
        required_response = {
            "schema_version", "fixture_id", "fixture_digest", "shared_answer_binding",
            "shared_answer_binding_digest",
            "answer_payload", "model_receipts", "gate_receipts", "execution_order",
            "integration_binding", "external_provider_called", "fixture_replay_only",
            "runtime_integrated", "production_deployed",
        }
        if not isinstance(response, Mapping) or set(response) != required_response or response.get("production_deployed") is not False or response.get("external_provider_called") is not False or response.get("fixture_replay_only") is not True or response.get("runtime_integrated") is not True:
            raise W5InvestigationError("model_turn_invalid", "model port 响应未通过 W5 边界")
        answer_payload = response.get("answer_payload")
        model_receipts = response.get("model_receipts")
        gate_receipts = response.get("gate_receipts")
        if (
            response.get("shared_answer_binding") != binding
            or response.get("shared_answer_binding_digest") != binding_digest
            or not isinstance(answer_payload, Mapping)
            or set(answer_payload) != {"claims", "limitations", "unknowns", "evidence_refs", "answer_text", "answer_payload_digest"}
            or answer_payload.get("answer_payload_digest") != digest_prefixed({key: answer_payload[key] for key in answer_payload if key != "answer_payload_digest"})
            or not isinstance(model_receipts, list) or len(model_receipts) != 3
            or not isinstance(gate_receipts, list) or len(gate_receipts) != 4
        ):
            raise W5InvestigationError("model_turn_artifact_invalid", "model/gate 回执或回答摘要不闭合")
        sidecar_receipts = [*model_receipts, *gate_receipts]
        for receipt in sidecar_receipts:
            if (
                not isinstance(receipt, Mapping)
                or receipt.get("shared_answer_binding_digest") != binding_digest
                or not isinstance(receipt.get("source_composition_receipt_digest"), str)
                or re.fullmatch(r"sha256:[0-9a-f]{64}", receipt["source_composition_receipt_digest"]) is None
                or receipt.get("external_provider_called") is not False
                or receipt.get("fixture_replay_only") is not True
                or receipt.get("receipt_digest") != digest_prefixed({field: receipt[field] for field in receipt if field != "receipt_digest"})
            ):
                raise W5InvestigationError("model_turn_receipt_invalid", "model/Host Gate receipt 无法重算")
        if len({item["receipt_digest"] for item in model_receipts}) != 3 or len({item["receipt_digest"] for item in gate_receipts}) != 4:
            raise W5InvestigationError("model_turn_receipt_duplicate", "model/Host Gate receipt 摘要必须逐项唯一")
        model_by_role = {item["model_role"]: item for item in model_receipts}
        gate_by_id = {item["receipt_kind"]: item for item in gate_receipts}
        if (
            set(model_by_role) != {"sol-planning", "sol-teacher", "ds-student"}
            or model_by_role["sol-planning"].get("subject_digest") != request["question_digest"]
            or model_by_role["sol-teacher"].get("subject_digest") != request["question_digest"]
            or model_by_role["ds-student"].get("subject_digest") != answer_payload["answer_payload_digest"]
            or set(gate_by_id) != {"composition_plan_grounding_gate", "composition_teacher_gate", "composition_student_gate", "composition_alignment_gate"}
            or gate_by_id["composition_plan_grounding_gate"].get("subject_digest") != binding_digest
            or gate_by_id["composition_teacher_gate"].get("subject_digest") != graph_ref["graph_digest"]
            or gate_by_id["composition_student_gate"].get("subject_digest") != host_oracle["oracle_digest"]
            or gate_by_id["composition_alignment_gate"].get("subject_digest") != answer_payload["answer_payload_digest"]
        ):
            raise W5InvestigationError("model_turn_receipt_subject_drift", "Sidecar model/gate receipt 未绑定当前回答闭包")
        integration = response.get("integration_binding")
        expected_integration = {
            "investigation_id": investigation_id,
            "source_investigation_revision": revision,
            "source_current_digest": expected,
            "plan_digest": plan_ref["plan_digest"],
            "result_set_refs_digest": digest_prefixed(result_refs),
            "graph_digest": graph_ref["graph_digest"],
            "oracle_digest": host_oracle["oracle_digest"],
            "binding_generation": value["identity"]["binding_generation"],
        }
        if integration != expected_integration:
            raise W5InvestigationError("model_turn_integration_binding_drift", "Sidecar flow 未绑定当前 Investigation 制品闭包")
        graph_fact_index = {fact["fact_id"]: fact for fact in graph_facts}
        claims = answer_payload.get("claims")
        if not isinstance(claims, list) or not claims:
            raise W5InvestigationError("model_turn_claims_missing", "回答必须包含 typed claims")
        answer_text = answer_payload.get("answer_text")
        if not isinstance(answer_text, str) or not answer_text.strip():
            raise W5InvestigationError("model_turn_prohibited_answer_text", "回答正文为空")
        claim_ids = [claim.get("claim_id") for claim in claims if isinstance(claim, Mapping)]
        if len(claim_ids) != len(claims) or any(not isinstance(claim_id, str) or not claim_id for claim_id in claim_ids):
            raise W5InvestigationError("model_turn_claim_id_invalid", "typed claim_id 必须是非空 Host 可重算标识")
        if len(set(claim_ids)) != len(claims):
            raise W5InvestigationError("model_turn_claim_id_duplicate", "typed claim_id 必须唯一")
        relation_by_kind = {
            "observed_fact": "states_observed_fact", "derived_fact": "states_derived_fact",
            "knowledge_explanation": "explains_knowledge", "testable_hypothesis": "proposes_testable_hypothesis",
            "limitation": "states_limitation", "unknown": "states_unknown",
        }
        claim_fields = {
            "claim_id", "claim_kind", "claim_relation", "text", "fact_ids", "source_node_ids",
            "source_value_digests", "evidence_refs", "boundary_assertion_ids", "verification_requirements",
        }
        for claim in claims:
            if not isinstance(claim, Mapping) or set(claim) != claim_fields or claim.get("claim_kind") not in relation_by_kind or claim.get("claim_relation") != relation_by_kind[claim["claim_kind"]]:
                raise W5InvestigationError("model_turn_claim_invalid", "typed claim 字段或 kind/relation 不闭合")
            if not isinstance(claim.get("text"), str) or not claim["text"].strip() or _PROHIBITED_ANSWER_CLAIM.search(claim["text"]):
                raise W5InvestigationError("model_turn_prohibited_claim", "回答 claim 包含越界语义")
            arrays = {field: claim.get(field) for field in claim_fields - {"claim_id", "claim_kind", "claim_relation", "text"}}
            if any(not isinstance(items, list) or any(not isinstance(item, str) or not item for item in items) for items in arrays.values()):
                raise W5InvestigationError("model_turn_claim_array_invalid", "typed claim 引用数组无效")
            if claim["claim_kind"] not in {"observed_fact", "derived_fact"}:
                raise W5InvestigationError("model_turn_untrusted_free_text_claim", "W5 发布 claims 只接受 Host 确定性生成的 Graph 事实文本")
            if claim["claim_kind"] in {"observed_fact", "derived_fact"}:
                if len(claim["fact_ids"]) != 1 or claim["fact_ids"][0] not in graph_fact_index:
                    raise W5InvestigationError("model_turn_fact_binding_invalid", "事实 claim 未绑定唯一当前 Graph fact")
                fact = graph_fact_index[claim["fact_ids"][0]]
                if claim["source_node_ids"] != [fact["source_node_id"]] or claim["source_value_digests"] != [fact["source_value_digest"]] or claim["evidence_refs"] != fact["evidence_refs"]:
                    raise W5InvestigationError("model_turn_fact_value_evidence_drift", "事实 claim 的 node/value/Evidence 漂移")
                if claim["claim_kind"] != fact["claim_kind"] or claim["claim_relation"] != fact["claim_relation"]:
                    raise W5InvestigationError("model_turn_fact_kind_drift", "事实 claim 的 kind/relation 与 Graph fact 不一致")
                expected_claim_id = "claim-sha256:" + digest_hex({"binding": binding_digest, "fact_id": fact["fact_id"]})
                if claim["claim_id"] != expected_claim_id:
                    raise W5InvestigationError("model_turn_claim_id_drift", "事实 claim_id 未绑定当前 shared binding 与 Graph fact")
                if claim["text"] != fact["allowed_claim_text"]:
                    raise W5InvestigationError("model_turn_fact_text_drift", "事实 claim 文本不是 Host 对当前 Graph fact 的确定性投影")
                if claim["verification_requirements"]:
                    raise W5InvestigationError("model_turn_fact_verification_text_forbidden", "已绑定事实 claim 不得附加模型自由验证文本")
                if not claim["boundary_assertion_ids"] or any(item not in host_oracle["boundary_assertions"] for item in claim["boundary_assertion_ids"]):
                    raise W5InvestigationError("model_turn_boundary_binding_invalid", "事实 claim 未绑定 Host boundary assertion")
        if answer_payload.get("limitations") != host_oracle["limitations"] or answer_payload.get("unknowns") != host_oracle["unknowns"]:
            raise W5InvestigationError("model_turn_oracle_boundary_drift", "回答 limitations/unknowns 未绑定当前 Host Oracle")
        expected_answer_text = " ".join([
            *(claim["text"] for claim in claims),
            *host_oracle["limitations"],
            *host_oracle["unknown_texts"],
        ])
        if answer_text != expected_answer_text:
            raise W5InvestigationError("model_turn_answer_text_drift", "回答正文不是 Host fact、限制与未知项的确定性投影")
        claimed_evidence = sorted({item for claim in claims for item in claim["evidence_refs"]})
        if answer_payload.get("evidence_refs") != claimed_evidence or any(item not in evidence_refs for item in claimed_evidence):
            raise W5InvestigationError("model_turn_evidence_drift", "回答 evidence refs 越出 Graph 闭包")
        receipt_binding = {
            "investigation_id": investigation_id,
            "investigation_revision": revision + 1,
            "source_current_digest": expected,
            "owner_principal_id": value["owner_principal_id"],
            "answer_payload_digest": answer_payload["answer_payload_digest"],
            "plan_digest": plan_ref["plan_digest"],
            "result_set_refs_digest": digest_prefixed(result_refs),
            "graph_digest": graph_ref["graph_digest"],
            "shared_answer_binding_digest": binding_digest,
        ...
        normalized_model_receipts = []
        for source in model_receipts:
            base = {
                "receipt_kind": "model", "receipt_version": 1,
                **receipt_binding,
                "model_role": source["model_role"],
                "model_identity": source["model_identity"],
                "source_sidecar_receipt_digest": source["receipt_digest"],
                "source_composition_receipt_digest": source["source_composition_receipt_digest"],
                "external_provider_called": False, "fixture_replay_only": True,
                "disposition": "passed",
            }
            normalized_model_receipts.append({**base, "receipt_digest": digest_prefixed(base)})
        normalized_gate_receipts = []
        for source in gate_receipts:
            base = {
                "receipt_kind": "gate", "receipt_version": 1,
                **receipt_binding,
                "gate_id": source["receipt_kind"],
                "source_sidecar_receipt_digest": source["receipt_digest"],
                "source_composition_receipt_digest": source["source_composition_receipt_digest"],
                "external_provider_called": False, "fixture_replay_only": True,
                "disposition": "passed",
            }
            normalized_gate_receipts.append({**base, "receipt_digest": digest_prefixed(base)})
        all_receipts = [*normalized_model_receipts, *normalized_gate_receipts]
        answer_base = {
            "schema_version": "country_outage_p2_s1_w5_answer_v1",
            "answer_revision": 1,
            "parent_answer_revision": None,
            "investigation_id": investigation_id,
            "source_investigation_revision": revision,
            "source_current_digest": expected,
            "committed_investigation_revision": revision + 1,
            "plan_ref": plan_ref,
            "result_set_refs": result_refs,
            "evidence_graph_ref": graph_ref,
            "shared_answer_binding": binding,
            "shared_answer_binding_digest": binding_digest,
            "claims": copy.deepcopy(answer_payload["claims"]),
            "limitations": copy.deepcopy(answer_payload["limitations"]),
            "unknowns": copy.deepcopy(host_oracle["unknown_texts"]),
            "unknown_ids": copy.deepcopy(answer_payload["unknowns"]),
            "evidence_refs": copy.deepcopy(answer_payload["evidence_refs"]),
            "answer_text": answer_payload["answer_text"],
            "model_receipt_digests": [item["receipt_digest"] for item in normalized_model_receipts],
            "gate_receipt_digests": [item["receipt_digest"] for item in normalized_gate_receipts],
            "fixture_boundary": {
                "fixture_id": response["fixture_id"],
                "fixture_digest": response["fixture_digest"],
        ...
        answer_id = "answer_" + digest_hex(answer_base)
        answer = {**answer_base, "answer_id": answer_id}
        answer["answer_digest"] = digest_prefixed(answer)
        turn_base = {
            "schema_version": "country_outage_p2_s1_w5_turn_v1",
            "turn_revision": 1,
            "parent_turn_revision": None,
            "investigation_id": investigation_id,
            "source_investigation_revision": revision,
            "source_current_digest": expected,
            "committed_investigation_revision": revision + 1,
            ...
            "anchor": copy.deepcopy(dict(anchor)),
            "shared_answer_binding_digest": binding_digest,
            "answer_ref": {"answer_id": answer_id, "answer_revision": 1, "answer_digest": answer["answer_digest"]},
        }
        turn_id = "turn_" + digest_hex(turn_base)
        turn = {**turn_base, "turn_id": turn_id}
        turn["turn_digest"] = digest_prefixed(turn)
        answer_object_digest = digest_prefixed(answer)
        turn_object_digest = digest_prefixed(turn)
        ref = {
            "turn_id": turn_id, "turn_revision": 1, "object_digest": turn_object_digest,
            "turn_digest": turn["turn_digest"], "answer_id": answer_id, "answer_revision": 1,
            "answer_object_digest": answer_object_digest, "answer_digest": answer["answer_digest"],
            "idempotency_key_digest": digest_prefixed({"idempotency_key": key}),
        }
        # 内容寻址对象先以不可覆盖字节发布，再让 investigation current pointer 通过
        # 单一 CAS 引用它们。CAS 失败时对象只是不可达 staging residue，不会成为
        # API 可消费 ghost；CAS 成功时所有引用已可读。
        if self.store.put_json("model-answer", answer)["object_digest"] != answer_object_digest or self.store.put_json("model-turn", turn)["object_digest"] != turn_object_digest:
            raise W5InvestigationError("turn_store_digest_mismatch", "Turn/Answer Store 摘要漂移")
        for receipt in all_receipts:
            self.store.put_json("receipt", receipt)
        snapshot = {**value, "investigation_revision": revision + 1, "parent_investigation_revision": revision, "turn_refs": [*value.get("turn_refs", []), ref]}
        committed, replayed = self._commit(snapshot=snapshot, expected_digest=expected, expected_revision=revision, idempotency_key=key, request=body)
        if replayed:
            existing_ref = next((item for item in committed.get("turn_refs", []) if item.get("idempotency_key_digest") == ref["idempotency_key_digest"]), None)
            if existing_ref is None:
                raise W5InvestigationError("turn_idempotency_outcome_invalid", "追问重放缺少 turn ref")
            turn = self._turn_from_ref(committed, existing_ref)
        else:
            turn = {**turn, "answer": answer}
        live = self.store.read_pointer("investigation", investigation_id)
        return {"accepted": True, "deduplicated": replayed, "turn": self._turn_view(turn), "investigation": self._view(committed, live["object_digest"])}, 202

    def _turn_from_ref(self, investigation: Mapping[str, Any], ref: Mapping[str, Any]) -> dict[str, Any]:
        turn = self.store.get_json("model-turn", ref["object_digest"])
        answer = self.store.get_json("model-answer", ref["answer_object_digest"])
        if (
            turn.get("investigation_id") != investigation["investigation_id"]
            or turn.get("turn_id") != ref["turn_id"]
            or turn.get("turn_revision") != ref["turn_revision"]
            or turn.get("turn_digest") != ref["turn_digest"]
            or turn.get("turn_digest") != digest_prefixed({field: turn[field] for field in turn if field != "turn_digest"})
            or answer.get("answer_id") != ref["answer_id"]
            or answer.get("answer_revision") != ref["answer_revision"]
            or answer.get("answer_digest") != ref["answer_digest"]
            or answer.get("answer_digest") != digest_prefixed({field: answer[field] for field in answer if field != "answer_digest"})
            or turn.get("answer_ref", {}).get("answer_digest") != answer["answer_digest"]
        ):
            raise W5InvestigationError("turn_answer_closure_invalid", "Turn/Answer trusted closure 无法重算")
        return {**turn, "answer": answer}

    @staticmethod
    def _turn_view(turn: Mapping[str, Any]) -> dict[str, Any]:
        answer = turn["answer"]
        return {
            "turn_id": turn["turn_id"], "turn_revision": turn["turn_revision"],
            "investigation_id": turn["investigation_id"],
            "source_investigation_revision": turn["source_investigation_revision"],
            "source_current_digest": turn["source_current_digest"],
            "committed_investigation_revision": turn["committed_investigation_revision"],
            "question": turn["question"], "anchor": copy.deepcopy(turn["anchor"]),
            ...
            "answer": {
                ...
                for key in (
                    ...
                    "claims", "limitations", "unknowns", "unknown_ids", "evidence_refs", "answer_text",
                    "plan_ref", "result_set_refs", "evidence_graph_ref", "model_receipt_digests",
                    "gate_receipt_digests", "fixture_boundary",
                )
            },
        }

    def get_turn(self, principal: Mapping[str, Any], investigation_id: str, turn_id: str, turn_revision: int):
        _, value, _ = self._owned(principal, investigation_id)
        refs = [item for item in value.get("turn_refs", []) if item.get("turn_id") == turn_id and item.get("turn_revision") == turn_revision]
        if len(refs) != 1:
            raise LookupError("Turn revision 不属于调查")
        return {"turn": self._turn_view(self._turn_from_ref(value, refs[0]))}
    ...
    def get_receipts(self, principal: Mapping[str, Any], investigation_id: str, query: Mapping[str, Any]):
        actor, value, _ = self._owned(principal, investigation_id)
        allowed_digests = {
            str(item.get("receipt_digest")) for item in value.get("nodes", []) if item.get("receipt_digest")
        } | {
            str(item.get("digest")) for item in value.get("receipt_refs", []) if item.get("digest")
        }
        for ref in value.get("turn_refs", []):
            answer = self.store.get_json("model-answer", ref["answer_object_digest"])
            allowed_digests.update(answer.get("model_receipt_digests", []))
            allowed_digests.update(answer.get("gate_receipt_digests", []))
        receipts = [
            item for item in self.store.list_json("receipt")
            if item.get("investigation_id") == investigation_id and item.get("receipt_digest") in allowed_digests
        ]
        for item in receipts:
            if item.get("receipt_kind") in {"model", "gate"} and (
                item.get("owner_principal_id") != actor["principal_id"]
                or not isinstance(item.get("receipt_version"), int)
                or item["receipt_version"] < 1
                or not isinstance(item.get("investigation_revision"), int)
                or item["investigation_revision"] > value["investigation_revision"]
            ):
                raise W5InvestigationError("answer_receipt_binding_invalid", "回答 receipt owner/version 绑定无效")
        kind = query.get("kind")
        if kind is not None:
...
__all__ = [
    ...
    "build_local_fixture_sidecar_runtime",
