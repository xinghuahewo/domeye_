...
from utils.get_as_info import get_as_country_cn
...
# 双槽 checkpoint：current 为最新快照，previous 作为最近一次可回退状态。
CHECKPOINT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../logs/checkpoints/feature')
CURRENT_CHECKPOINT_META_FILE = os.path.join(CHECKPOINT_DIR, 'current.json')
PREVIOUS_CHECKPOINT_META_FILE = os.path.join(CHECKPOINT_DIR, 'previous.json')
CURRENT_SNAPSHOT_FILE = os.path.join(CHECKPOINT_DIR, 'snapshot_current.pkl.gz')
PREVIOUS_SNAPSHOT_FILE = os.path.join(CHECKPOINT_DIR, 'snapshot_previous.pkl.gz')
CHECKPOINT_VERSION = 1
CHECKPOINT_STATUS_READY = 'ready'
CHECKPOINT_SNAPSHOT_FORMAT = 'pickle+gzip'
CHECKPOINT_FEATURE_NAME = 'BGPFeature'
...
class BGPFeature:
    ...
    @classmethod
    def from_checkpoint(cls, rib_file, conn, checkpoint_state):
        # 启动时优先走这里恢复，避免重新加载 bview 再逐个 replay 历史 update。
        # checkpoint_state 顶层只包含三块：
        # 1. bgp_rib: 当前维护的路由状态快照
        # 2. feature_dict: 已累计到当前时刻的国家 -> ASN 特征聚合
        # 3. feature_collect_dict: collect 维度的聚合计数
        obj = cls.__new__(cls)
        obj.conn = conn
        obj.rib_file = rib_file
        obj.bgp_info = BGPInfo()
        obj.bgp_rib = BGPRib(obj.bgp_info, rib_file, load_rib=False)
        obj.bgp_rib.load_state(checkpoint_state['bgp_rib'])
        obj.feature_dict = {
            country: {
                asn: dict(feature)
                for asn, feature in asn_dict.items()
            }
            for country, asn_dict in checkpoint_state.get('feature_dict', {}).items()
        }
        obj.feature_collect_dict = dict(checkpoint_state.get('feature_collect_dict', {}))
        obj._ensured_feature_asn_tables = set()
        return obj

    def dump_checkpoint_state(self):
        # 只保存恢复必需状态，不把数据库连接、logger、表缓存等运行时对象写入快照。
        # 这里的“内存态”指的是 BGPFeature 当前已经维护出来的核心业务状态：
        # - bgp_rib: 路由前缀、origin、VP 路径和 ASN -> prefix 映射
        # - feature_dict: 各国家下各 ASN 的累计特征
        # - feature_collect_dict: 全局 collect 计数
        return {
            'bgp_rib': self.bgp_rib.dump_state(),
            'feature_dict': {
                country: {
                    asn: dict(feature)
                    for asn, feature in asn_dict.items()
            ...
            'feature_collect_dict': self.feature_collect_dict.copy(),
        }
...
def _write_bytes_atomically(path, payload):
...
def _write_snapshot_atomically(path, state):
    _ensure_checkpoint_dir()
    tmp_path = f"{path}.tmp"
    with gzip.open(tmp_path, 'wb') as snapshot_file:
        pickle.dump(state, snapshot_file, protocol=pickle.HIGHEST_PROTOCOL)
...
def _read_snapshot_file(path):
    with gzip.open(path, 'rb') as snapshot_file:
        return pickle.load(snapshot_file)


def _build_checkpoint_meta(snapshot_path, last_completed_update_file, rib_file, rib_time):
    return {
        'version': CHECKPOINT_VERSION,
        'status': CHECKPOINT_STATUS_READY,
        'created_at': datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ'),
        'snapshot_path': snapshot_path,
        'last_completed_update_file': last_completed_update_file,
        'rib_file': rib_file,
        'rib_time': rib_time,
        'snapshot_format': CHECKPOINT_SNAPSHOT_FORMAT,
        'feature_name': CHECKPOINT_FEATURE_NAME,
    }


def _rotate_current_checkpoint_to_previous():
    # 写入新 current 之前，先把旧 current 挪到 previous，保留一个稳定回退点。
    _ensure_checkpoint_dir()
    current_meta = None
    if os.path.exists(CURRENT_CHECKPOINT_META_FILE):
        try:
            current_meta = _read_json_file(CURRENT_CHECKPOINT_META_FILE)
        except Exception as exc:
            feature_logger.error(f"读取当前checkpoint metadata失败，跳过previous metadata轮转: {exc}")

    _remove_file_if_exists(PREVIOUS_CHECKPOINT_META_FILE)
    _remove_file_if_exists(PREVIOUS_SNAPSHOT_FILE)

    if os.path.exists(CURRENT_SNAPSHOT_FILE):
        os.replace(CURRENT_SNAPSHOT_FILE, PREVIOUS_SNAPSHOT_FILE)

    if current_meta and os.path.exists(PREVIOUS_SNAPSHOT_FILE):
        current_meta['snapshot_path'] = PREVIOUS_SNAPSHOT_FILE
        _write_json_atomically(PREVIOUS_CHECKPOINT_META_FILE, current_meta)


def commit_checkpoint(feature_detector, last_completed_update_file):
    # 只有完整处理完一个 update 文件后才提交 checkpoint。
    # 语义上恢复到 last_completed_update_file，对处理中途崩溃的文件允许整文件重跑。
    _ensure_checkpoint_dir()
    checkpoint_state = feature_detector.dump_checkpoint_state()
    _write_snapshot_atomically(f"{CURRENT_SNAPSHOT_FILE}.next", checkpoint_state)
    try:
        _rotate_current_checkpoint_to_previous()
        os.replace(f"{CURRENT_SNAPSHOT_FILE}.next", CURRENT_SNAPSHOT_FILE)
        checkpoint_meta = _build_checkpoint_meta(
            snapshot_path=CURRENT_SNAPSHOT_FILE,
            last_completed_update_file=last_completed_update_file,
            rib_file=feature_detector.rib_file,
            rib_time=feature_detector.bgp_rib.t,
        )
        _write_json_atomically(CURRENT_CHECKPOINT_META_FILE, checkpoint_meta)
    finally:
        _remove_file_if_exists(f"{CURRENT_SNAPSHOT_FILE}.next")


def _load_checkpoint_slot(meta_path, snapshot_path):
    if not os.path.exists(meta_path) or not os.path.exists(snapshot_path):
        return None

    try:
        checkpoint_meta = _read_json_file(meta_path)
        if checkpoint_meta.get('version') != CHECKPOINT_VERSION:
            raise ValueError(f"unsupported checkpoint version: {checkpoint_meta.get('version')}")
        if checkpoint_meta.get('status') != CHECKPOINT_STATUS_READY:
            raise ValueError(f"checkpoint status is not ready: {checkpoint_meta.get('status')}")
        if os.path.abspath(checkpoint_meta.get('snapshot_path', '')) != os.path.abspath(snapshot_path):
            raise ValueError('snapshot_path does not match slot path')

        checkpoint_state = _read_snapshot_file(snapshot_path)
        if 'bgp_rib' not in checkpoint_state:
            raise ValueError('checkpoint state missing bgp_rib')
        return checkpoint_meta, checkpoint_state
    except Exception as exc:
        feature_logger.error(f"加载checkpoint失败 {meta_path}: {exc}")
        feature_logger.error(traceback.format_exc())
        return None


def load_latest_checkpoint():
    # 恢复顺序固定为 current -> previous -> 无 checkpoint。
    current_checkpoint = _load_checkpoint_slot(CURRENT_CHECKPOINT_META_FILE, CURRENT_SNAPSHOT_FILE)
    if current_checkpoint is not None:
        feature_logger.info(f"从current checkpoint恢复: {CURRENT_CHECKPOINT_META_FILE}")
        return current_checkpoint

    previous_checkpoint = _load_checkpoint_slot(PREVIOUS_CHECKPOINT_META_FILE, PREVIOUS_SNAPSHOT_FILE)
    if previous_checkpoint is not None:
        feature_logger.info(f"从previous checkpoint恢复: {PREVIOUS_CHECKPOINT_META_FILE}")
...
def main():
    ...
    try:
        ...
        # 有可用 checkpoint 时，直接恢复内存态；只有两槽都不可用时才回退到 bview 初始化。
        if checkpoint is not None:
            checkpoint_meta, checkpoint_state = checkpoint
            feature_detector = BGPFeature.from_checkpoint(
                rib_file=checkpoint_meta.get('rib_file', rib_file),
                conn=conn,
                checkpoint_state=checkpoint_state,
            )
            is_init = True
            last_processed = checkpoint_meta.get('last_completed_update_file')
            feature_logger.info(f"checkpoint恢复成功，last_completed_update_file={last_processed}")
        else:
            start = time.time()
        ...
        if last_processed:
            ...
            feature_logger.info(f"恢复时跳过已处理的{skipped_count}个文件，从 {update_list[0] if update_list else 'N/A'} 继续")
    ...
    # TODO: try catch exit
        while True:
            ...
            while len(Updates_File_Queue) > 0:
                ...
                update_success = feature_detector.update_checking(updates_file=updates_file)
                remove_file(updates_file)
                if not update_success:
                    raise RuntimeError(f"处理update文件失败: {updates_file_original}")
                # 先提交 checkpoint，再更新旧 progress 文件，保证恢复优先以快照为准。
                commit_checkpoint(feature_detector, updates_file_original)
                end = int(time.time())
            ...
            # # 防止在某些极端情况下 有一个文件丢失或者下载失败（一般不会） 导致程序卡死
            # if is_handle_file == False and len(Updates_File_Queue) > 0:
            #     count += 1
            #     if count >= 10:
            #         count = 0
            #         feature_logger.info(f"没有找到{Current_Updates_File}")
            #         # 当前需要的文件小于队头
            #         if Current_Updates_File < Updates_File_Queue[0]:
            #             Last_File_Name = Current_Updates_File
            #             Current_Updates_File_basename = get_current_file_name_from_updates(Last_File_Name)
            #             Current_Updates_File = get_update_file_abspath(Current_Updates_File_basename)
            #             feature_logger.info(f"当前需要updates文件为{Current_Updates_File}") 
            #         else:
            #             # 当前需要的文件大于队头  出队
            #             while len(Updates_File_Queue) and Current_Updates_File > Updates_File_Queue[0]:
            #                 name = Updates_File_Queue.pop(0)
            #                 feature_logger.info(f"出队文件:{name}")
            #             feature_logger.info(f"当前需要updates文件为{Current_Updates_File}")
            # else:
            #     is_handle_file = False
            #     count = 0
