...
class FakeInvestigationRuntime:
    ...
    def create_turn(self, principal, investigation_id, body):
        ...
        return {
            ...
            "deduplicated": False,
            "turn": {
                "turn_id": "turn_1", "turn_revision": 1, "investigation_id": investigation_id,
                "source_investigation_revision": 2, "source_current_digest": DIGEST,
                "committed_investigation_revision": 3, "question": body["question"],
                "anchor": body["anchor"], "turn_digest": DIGEST,
                "answer": {
                    "answer_id": "answer_1", "answer_revision": 1, "answer_digest": DIGEST,
                    "shared_answer_binding_digest": DIGEST,
                    "claims": [{"claim_id": "claim_1", "claim_kind": "observed_fact", "claim_relation": "states_observed_fact", "text": "已绑定证据。", "fact_ids": ["fact:1"], "source_node_ids": ["node_1"], "source_value_digests": [DIGEST], "evidence_refs": ["evidence:1"], "boundary_assertion_ids": ["boundary:1"], "verification_requirements": [], "secret": "drop"}],
                    "limitations": ["仅 RRC25"], "unknowns": ["原因未知"], "unknown_ids": ["unknown:1"],
                    "evidence_refs": ["evidence:1"], "answer_text": "已绑定证据。",
                    "plan_ref": {"plan_id": "plan_1", "plan_revision": 1, "plan_digest": DIGEST},
                    "result_set_refs": [],
                    "evidence_graph_ref": {"graph_id": "graph_1", "graph_revision": 1, "graph_digest": DIGEST},
                    "model_receipt_digests": [DIGEST], "gate_receipt_digests": [DIGEST],
                    "fixture_boundary": {"fixture_id": "fixture", "fixture_digest": DIGEST, "external_provider_called": False, "fixture_replay_only": True, "runtime_integrated": True, "production_deployed": False},
    ...
    def get_turn(self, principal, investigation_id, turn_id, turn_revision):
        body = {
            "idempotency_key": "w5-get-turn", "expected_investigation_revision": 2,
            "expected_current_digest": DIGEST, "question": "读取", "anchor": {"node_id": "node_1", "node_revision": 2},
        }
        return {"turn": self.create_turn(principal, investigation_id, body)[0]["turn"]}
...
class CountryOutageP2S1InvestigationApiTest(unittest.TestCase):
    ...
    def test_complete_local_journey_routes_are_thin_and_identity_bound(self):
        ...
        self.assertEqual(
            self.client.get("/api/v2/country-outage/investigations/inv_1/turns/turn_1/revisions/1").status_code,
            200,
        )
        self.assertEqual(
            self.client.get(
    ...
    def test_public_projection_drops_unversioned_turn_and_graph_payload(self):
        ...
        self.assertEqual(payload["turn"]["turn_revision"], 1)
        self.assertNotIn("raw_response", payload["turn"])
        self.assertNotIn("raw_model_output", payload["turn"]["answer"])
        self.assertNotIn("secret", payload["turn"]["answer"]["claims"][0])
