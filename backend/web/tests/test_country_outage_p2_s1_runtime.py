...
class IntegratedFixturePort:
    """Host 单测用受控端口；生产实现只允许 LocalFixtureSidecarModelPort。"""

    def __init__(self, mutate=None):
        self.mutate = mutate

    @staticmethod
    def receipt(kind, role, binding, subject):
        base = {
            "receipt_kind": kind, "model_role": role, "model_identity": f"local-fixture-{role}-v1",
            "shared_answer_binding_digest": binding, "subject_digest": subject,
            "source_composition_receipt_digest": digest_prefixed({"kind": kind, "role": role, "subject": subject}),
            "external_provider_called": False, "fixture_replay_only": True, "disposition": "passed",
        }
        return {**base, "receipt_digest": digest_prefixed(base)}

    def create_turn(self, request):
        fact = request["host_graph_facts"][0]
        oracle = request["host_oracle"]
        binding = request["shared_answer_binding_digest"]
        claim = {
            "claim_id": "claim-sha256:" + hashlib.sha256(canonical_json({"binding": binding, "fact_id": fact["fact_id"]}).encode("utf-8")).hexdigest(),
            "claim_kind": fact["claim_kind"], "claim_relation": fact["claim_relation"],
            "text": fact["allowed_claim_text"],
            "fact_ids": [fact["fact_id"]], "source_node_ids": [fact["source_node_id"]],
            "source_value_digests": [fact["source_value_digest"]], "evidence_refs": fact["evidence_refs"],
            "boundary_assertion_ids": oracle["boundary_assertions"], "verification_requirements": [],
        }
        answer_base = {
            "claims": [claim], "evidence_refs": fact["evidence_refs"],
            "limitations": oracle["limitations"], "unknowns": oracle["unknowns"],
            "answer_text": " ".join([
                claim["text"], *oracle["limitations"], *oracle["unknown_texts"],
            ]),
        }
        answer = {**answer_base, "answer_payload_digest": digest_prefixed(answer_base)}
        models = [
            self.receipt("composition_model_run", "sol-planning", binding, request["question_digest"]),
            self.receipt("composition_model_run", "sol-teacher", binding, request["question_digest"]),
            self.receipt("composition_model_run", "ds-student", binding, answer["answer_payload_digest"]),
        ]
        gates = [self.receipt(kind, "sidecar-host", binding, subject) for kind, subject in (
            ("composition_plan_grounding_gate", binding), ("composition_teacher_gate", request["evidence_graph_ref"]["graph_digest"]),
            ("composition_student_gate", oracle["oracle_digest"]), ("composition_alignment_gate", answer["answer_payload_digest"]),
        )]
        response = {
            "schema_version": "country_outage_p2_s1_w5_model_turn_v1", "fixture_id": "fixture-integrated",
            "fixture_digest": digest_prefixed("fixture-integrated"), "shared_answer_binding": request["shared_answer_binding"],
            "shared_answer_binding_digest": binding, "answer_payload": answer,
            "model_receipts": models, "gate_receipts": gates,
            "execution_order": ["gpt-5.6-sol-fixture", "host-grounding", "ds-student-fixture", "host-publish-gates"],
            "integration_binding": {
                "investigation_id": request["investigation_id"], "source_investigation_revision": request["source_investigation_revision"],
                "source_current_digest": request["source_current_digest"], "plan_digest": request["plan_ref"]["plan_digest"],
                "result_set_refs_digest": digest_prefixed(request["result_set_refs"]), "graph_digest": request["evidence_graph_ref"]["graph_digest"],
                "oracle_digest": oracle["oracle_digest"], "binding_generation": request["identity"]["binding_generation"],
            },
            "external_provider_called": False, "fixture_replay_only": True, "runtime_integrated": True,
            "production_deployed": False,
        }
        if self.mutate:
            self.mutate(response)
            base = {key: response["answer_payload"][key] for key in response["answer_payload"] if key != "answer_payload_digest"}
            response["answer_payload"]["answer_payload_digest"] = digest_prefixed(base)
            for receipt in [response["model_receipts"][2], response["gate_receipts"][3]]:
                receipt["subject_digest"] = response["answer_payload"]["answer_payload_digest"]
                receipt["receipt_digest"] = digest_prefixed({key: receipt[key] for key in receipt if key != "receipt_digest"})
        return response
...
class CountryOutageP2S1RuntimeTest(unittest.TestCase):
    def setUp(self):
        ...
        self.runtime = build_local_fixture_runtime(
            ...
            model_port=IntegratedFixturePort(),
        )
    ...
    def test_python_fixture_turn_is_versioned_bound_and_idempotent(self):
        created = self.create("turn")
        completed = self.runtime.start_investigation(
            PRINCIPAL, created["investigation_id"], self.cas(created, "start-turn")
        )[0]["investigation"]
        anchor = next(item for item in completed["nodes"] if item["state"] == "committed")
        body = {
            **self.cas(completed, "turn-fixture-0001"),
            "question": "这个节点的证据边界是什么？",
            "anchor": {"node_id": anchor["node_id"], "node_revision": anchor["execution_revision"]},
        }
        first = self.runtime.create_turn(PRINCIPAL, completed["investigation_id"], body)[0]
        self.assertFalse(first["deduplicated"])
        self.assertEqual(first["turn"]["turn_revision"], 1)
        self.assertEqual(first["turn"]["source_investigation_revision"], completed["investigation_revision"])
        self.assertEqual(first["turn"]["source_current_digest"], completed["current_digest"])
        ...
        self.assertFalse(answer["fixture_boundary"]["external_provider_called"])
        self.assertTrue(answer["fixture_boundary"]["fixture_replay_only"])
        self.assertEqual(len(answer["model_receipt_digests"]), 3)
        self.assertGreaterEqual(len(answer["gate_receipt_digests"]), 4)
        self.assertEqual(first["investigation"]["turn_refs"][0]["turn_id"], first["turn"]["turn_id"])

        replay = self.runtime.create_turn(PRINCIPAL, completed["investigation_id"], body)[0]
        self.assertTrue(replay["deduplicated"])
        self.assertEqual(replay["turn"], first["turn"])
        loaded = self.runtime.get_turn(
            PRINCIPAL, completed["investigation_id"], first["turn"]["turn_id"], 1
        )["turn"]
        self.assertEqual(loaded, first["turn"])
        model_receipts = self.runtime.get_receipts(PRINCIPAL, completed["investigation_id"], {"kind": "model", "cursor": None})["receipts"]
        gate_receipts = self.runtime.get_receipts(PRINCIPAL, completed["investigation_id"], {"kind": "gate", "cursor": None})["receipts"]
        self.assertEqual({item["receipt_digest"] for item in model_receipts}, set(answer["model_receipt_digests"]))
        self.assertEqual({item["receipt_digest"] for item in gate_receipts}, set(answer["gate_receipt_digests"]))
        self.assertTrue(all(item["investigation_id"] == completed["investigation_id"] and item["owner_principal_id"] == PRINCIPAL["user_id"] and item["receipt_version"] == 1 for item in [*model_receipts, *gate_receipts]))
        with self.assertRaises(LookupError):
            self.runtime.get_turn(PRINCIPAL, completed["investigation_id"], first["turn"]["turn_id"], 2)
        with self.assertRaises(LookupError):
            self.runtime.get_turn(
                {"user_id": "other", "authorization_scope": "country_outage_event_read:IR"},
                completed["investigation_id"], first["turn"]["turn_id"], 1,
            )

    def test_turn_rejects_stale_revision_and_invalid_model_without_public_residue(self):
        created = self.create("turn-fail")
        completed = self.runtime.start_investigation(
            PRINCIPAL, created["investigation_id"], self.cas(created, "start-turn-fail")
        )[0]["investigation"]
        anchor = next(item for item in completed["nodes"] if item["state"] == "committed")
        stale = {
            **self.cas(completed, "turn-stale-0001"),
            "question": "旧节点还能回答吗？",
            "anchor": {"node_id": anchor["node_id"], "node_revision": anchor["execution_revision"] - 1},
        }
        with self.assertRaises(W5InvestigationError) as stale_error:
            self.runtime.create_turn(PRINCIPAL, completed["investigation_id"], stale)
        self.assertEqual(stale_error.exception.code, "turn_anchor_stale")

        class InvalidModelPort:
            def create_turn(self, request):
                return {"production_deployed": False, "external_provider_called": True}

        self.runtime.model_port = InvalidModelPort()
        invalid = {
            **self.cas(completed, "turn-invalid-0001"),
            "question": "不得伪造外部模型结果",
            "anchor": {"node_id": anchor["node_id"], "node_revision": anchor["execution_revision"]},
        }
        before_pointer = self.runtime.store.read_pointer("investigation", completed["investigation_id"])
        with self.assertRaises(W5InvestigationError) as invalid_error:
            self.runtime.create_turn(PRINCIPAL, completed["investigation_id"], invalid)
        self.assertEqual(invalid_error.exception.code, "model_turn_invalid")
        self.assertEqual(self.runtime.store.read_pointer("investigation", completed["investigation_id"]), before_pointer)
        self.assertEqual(self.runtime.store.list_json("model-turn"), [])
        self.assertEqual(self.runtime.store.list_json("model-answer"), [])

    def test_turn_artifacts_exist_before_public_cas_and_failed_cas_has_no_public_ref(self):
        created = self.create("turn-cas")
        completed = self.runtime.start_investigation(
            PRINCIPAL, created["investigation_id"], self.cas(created, "start-turn-cas")
        )[0]["investigation"]
        anchor = next(item for item in completed["nodes"] if item["state"] == "committed")
        body = {
            **self.cas(completed, "turn-cas-fail-01"),
            "question": "CAS 失败时是否公开回答？",
            "anchor": {"node_id": anchor["node_id"], "node_revision": anchor["execution_revision"]},
        }
        def fail_after_staging(*args, **kwargs):
            self.assertEqual(len(self.runtime.store.list_json("model-turn")), 1)
            self.assertEqual(len(self.runtime.store.list_json("model-answer")), 1)
            raise W5StoreError("compare_and_swap_conflict", "故障注入")

        with patch.object(self.runtime.store, "compare_and_swap_pointer", side_effect=fail_after_staging):
            with self.assertRaises(W5StoreError):
                self.runtime.create_turn(PRINCIPAL, completed["investigation_id"], body)
        live = self.runtime.get_investigation(PRINCIPAL, completed["investigation_id"])["investigation"]
        self.assertEqual(live["turn_refs"], [])
        self.assertEqual(live["current_digest"], completed["current_digest"])

        accepted = self.runtime.create_turn(PRINCIPAL, completed["investigation_id"], body)[0]
        self.assertFalse(accepted["deduplicated"])
        self.assertEqual(len(accepted["investigation"]["turn_refs"]), 1)

    def test_hostile_port_prohibited_claim_or_missing_unknowns_never_publishes_turn(self):
        created = self.create("hostile")
        completed = self.runtime.start_investigation(
            PRINCIPAL, created["investigation_id"], self.cas(created, "start-hostile")
        )[0]["investigation"]
        anchor = next(item for item in completed["nodes"] if item["state"] == "committed")
        for label, mutate, expected_code in (
            (
                "prohibited",
                lambda response: response["answer_payload"]["claims"][0].update({"text": "该事件导致全国用户影响并且已经恢复。"}),
                "model_turn_prohibited_claim",
            ),
            (
                "unknowns",
                lambda response: response["answer_payload"].update({"unknowns": []}),
                "model_turn_oracle_boundary_drift",
            ),
            (
                "limitations",
                lambda response: response["answer_payload"].update({"limitations": []}),
                "model_turn_oracle_boundary_drift",
            ),
            (
                "fact-value-drift",
                lambda response: response["answer_payload"]["claims"][0].update({"source_value_digests": ["sha256:" + "f" * 64]}),
                "model_turn_fact_value_evidence_drift",
            ),
            (
                "null-claim-id-with-free-verification-text",
                lambda response: response["answer_payload"]["claims"][0].update({
                    "claim_id": None,
                    "verification_requirements": ["声称整个国家民众无法上网且服务现已正常"],
                }),
                "model_turn_claim_id_invalid",
            ),
            (
                "duplicate-gate-receipt",
                lambda response: response["gate_receipts"].append(copy.deepcopy(response["gate_receipts"][0])),
                "model_turn_artifact_invalid",
            ),
            (
                "false-numeric-fact-text",
                lambda response: response["answer_payload"]["claims"][0].update({"text": "RRC25 在该绑定窗口观测到 999999 个前缀不可见。"}),
                "model_turn_fact_text_drift",
            ),
            (
                "commercial-causal-national-responsibility-recovery-paraphrase",
                lambda response: response["answer_payload"]["claims"][0].update({
                    "text": "该自治系统的付费下游网络使整个国家的民众无法使用互联网，运营商应当承担后果，目前服务已回到正常状态。",
                }),
                "model_turn_fact_text_drift",
            ),
            (
                "limitation-claim-free-text-bypass",
                lambda response: (
                    response["answer_payload"].update({
                        "answer_text": response["answer_payload"]["answer_text"].replace(
                            response["answer_payload"]["claims"][0]["text"],
                            "付费下游覆盖全国民众，运营商应承担后果，服务现已正常。",
                            1,
                        ),
                    }),
                    response["answer_payload"]["claims"][0].update({
                        "claim_kind": "limitation",
                        "claim_relation": "states_limitation",
                        "text": "付费下游覆盖全国民众，运营商应承担后果，服务现已正常。",
                        "fact_ids": [],
                        "source_node_ids": [],
                        "source_value_digests": [],
                        "evidence_refs": [],
                    }),
                    response["answer_payload"].update({"evidence_refs": []}),
                ),
                "model_turn_untrusted_free_text_claim",
            ),
            (
                "hypothesis-with-evidence",
                lambda response: response["answer_payload"]["claims"][0].update({
                    "claim_kind": "testable_hypothesis",
                    "claim_relation": "proposes_testable_hypothesis",
                    "verification_requirements": ["需要独立数据验证"],
                }),
                "model_turn_untrusted_free_text_claim",
            ),
        ):
            with self.subTest(label=label):
                self.runtime.model_port = IntegratedFixturePort(mutate)
                body = {
                    **self.cas(completed, f"turn-hostile-{label}"),
                    "question": "给出严格证据回答",
                    "anchor": {"node_id": anchor["node_id"], "node_revision": anchor["execution_revision"]},
                }
                with self.assertRaises(W5InvestigationError) as caught:
                    self.runtime.create_turn(PRINCIPAL, completed["investigation_id"], body)
                self.assertEqual(caught.exception.code, expected_code)
                current = self.runtime.get_investigation(PRINCIPAL, completed["investigation_id"])["investigation"]
                self.assertEqual(current["turn_refs"], [])
                self.assertEqual(current["current_digest"], completed["current_digest"])
        self.runtime.model_port = IntegratedFixturePort()

    def test_local_fixture_sidecar_model_port_consumes_real_node_composition_http(self):
        # Task 的正式 Python 检查先于 Sidecar 检查；每次按当前受信
        # TypeScript 字节构建 helper，禁止依赖工作树里偶然残留的旧 dist。
        build = subprocess.run(
            ["npm", "run", "build"],
            cwd=REPOSITORY_ROOT / "agent-sidecar",
            text=True,
            capture_output=True,
            check=False,
        )
        self.assertEqual(build.returncode, 0, build.stdout + build.stderr)
        created = self.create("node-http")
        completed = self.runtime.start_investigation(
            PRINCIPAL, created["investigation_id"], self.cas(created, "start-node-http")
        )[0]["investigation"]
        anchor = next(item for item in completed["nodes"] if item["state"] == "committed")
        token = "w5-python-node-http-token-at-least-24"
        env = {
            **os.environ,
            "COUNTRY_OUTAGE_P2_S1_W5_PYTHON_E2E_SERVER": "1",
            "COUNTRY_OUTAGE_P2_S1_W5_SHARED_TOKEN": token,
        }
        process = subprocess.Popen(
            ["node", "agent-sidecar/dist/tests/p2-s1-w5-integrated-answer-test-server.test.js"],
            cwd=REPOSITORY_ROOT,
            env=env,
        ...
        try:
            ...
            if not ready_line:
                ...
                self.fail(f"Node integrated answer server 未就绪：{stderr}")
            ready = json.loads(ready_line)
            fixture_digest = "sha256:" + "b" * 64
            self.runtime.model_port = __import__(
                "backend.services.country_outage_p2_s1_investigation_runtime",
                fromlist=["LocalFixtureSidecarModelPort"],
            ).LocalFixtureSidecarModelPort(
                base_url=f"http://127.0.0.1:{ready['port']}/country-outage/p2-s1-w5",
                shared_token=token,
                fixture_id="fixture-python-http-e2e",
                expected_fixture_digest=fixture_digest,
                expected_identity=completed["identity"],
            )
            body = {
                **self.cas(completed, "turn-node-http-0001"),
                "question": "从真实 CompositionRuntime 返回绑定事实回答",
                "anchor": {"node_id": anchor["node_id"], "node_revision": anchor["execution_revision"]},
            }
            response = self.runtime.create_turn(PRINCIPAL, completed["investigation_id"], body)[0]
            self.assertTrue(response["turn"]["answer"]["claims"])
            self.assertEqual(len(response["turn"]["answer"]["model_receipt_digests"]), 3)
            self.assertEqual(len(response["turn"]["answer"]["gate_receipt_digests"]), 4)
            self.assertFalse(response["turn"]["answer"]["fixture_boundary"]["external_provider_called"])
            self.assertEqual(len(response["investigation"]["turn_refs"]), 1)
        finally:
            process.terminate()
            try:
