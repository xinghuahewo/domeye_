...
def test_iran_observation_exposes_only_descriptive_data(story_package):
    observation = event_story_service.get_iran_event_observation(
        legacy_reference=event_story_service.IRAN_LEGACY_REF,
    ...
    assert observation["schema_version"] == "event_observation_v1"
...
def test_event_observation_api_combines_replay_and_core_resource_series(client):
    ...
    with patch(
        ...
        "web.api.events.api.get_iran_event_observation",
        return_value=payload,
    ...
    feature_query.assert_called_once_with(
        start_time=event_story_service.IRAN_WINDOW_START_LOCAL,
        end_time=event_story_service.IRAN_WINDOW_END_LOCAL,
        country="伊朗",
        page_num="1",
        page_size="5",
    )
