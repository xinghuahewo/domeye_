...
W6_RESEALED_TASK_ID = "country-outage-agent-p2-s1-w6-offline-certification-v3-20260814"
W6_RESEALED_TARGET_VERSION = "country-outage-agent-p2-s1-w6-offline-certification-v3"
...
def _w6_task_active(root: Path) -> bool:
    ...
    try:
        return load_json(root / TASK_PATH).get("taskId") in {
            W6_TASK_ID,
            W6_RESEALED_TASK_ID,
        }
...
def validate_task(root: Path) -> list[str]:
    ...
    w6_v2_task = task_id == W6_TASK_ID
    w6_resealed_task = task_id == W6_RESEALED_TASK_ID
    w6_task = w6_v2_task or w6_resealed_task
    expect(
        w5_task or w6_task,
        "task_identity_mismatch",
        "最终W5/W6验收只接受冻结的W5-v6、W6-v2或Guard重封存的W6-v3 Task",
    )
    if w6_task:
        expected_target_version = (
            W6_RESEALED_TARGET_VERSION if w6_resealed_task else W6_TARGET_VERSION
        )
        expected_superseded_task = W6_TASK_ID if w6_resealed_task else W5_TASK_ID
        expected_transition_kind = (
            "w6_final_contract_reseal_after_guard_detected_task_drift"
            if w6_resealed_task
            else "w5_independently_reviewed_candidate_to_w6_offline_deterministic_certification"
        )
        exact(task.get("targetVersion"), expected_target_version, "task_version_mismatch", "W6 目标版本不匹配")
        transition = task.get("taskTransition")
        expect(isinstance(transition, dict), "task_transition_missing", "W6 缺少任务迁移记录")
        for field, expected in {
            "transitionKind": expected_transition_kind,
            "supersedesTaskId": expected_superseded_task,
