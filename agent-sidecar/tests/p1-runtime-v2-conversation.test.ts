...
test('S4 责任、用户和经济影响复合目标全部按已登记政策零执行', async () => {
  ...
  assert.match(answer.results[0]?.text ?? '', /原因、责任主体或政府行为/)
  assert.match(answer.results[1]?.text ?? '', /不能量化受影响用户数/)
  assert.match(answer.results[2]?.text ?? '', /经济损失、金额或业务损失证据/)
  assert.doesNotMatch(answer.results[2]?.text ?? '', /责任主体/)
