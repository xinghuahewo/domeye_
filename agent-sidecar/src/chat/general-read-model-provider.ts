...
export interface P1AsnReadRequest {
  asn?: number | null
  query: string
...
export interface P1PathReadRequest {
  affectedAsn: number | null
...
export interface P1PageCapabilityReadProvider extends P1RuntimeV2ReadProvider {
  readSeries(
    binding: P1ConversationBinding,
    metrics: string[],
    signal?: AbortSignal,
...
implements P1GeneralReadModelProvider, P1PageCapabilityReadProvider {
  constructor(
  ...
  async readSeries(
    ...
    if (binding.capabilities.event_series !== 'available') {
      throw new P1ReadModelError(
        'capability_unavailable',
        '当前事件未协商 event_series=available',
      )
    }
    if (metrics.length === 0 || new Set(metrics).size !== metrics.length) {
      throw new P1ReadModelError('invalid_request', 'series metrics 必须非空且唯一')
    }
    const series = await this.get(
      `country-outages/${encodeURIComponent(binding.incident_id)}/series`,
      { publication_id: binding.publication_id },
      signal,
    )
    assertSameIdentity(series, binding, 'series')
    const tracks = object(series.tracks, 'series.tracks')
    for (const metric of metrics) {
      if (!Object.hasOwn(tracks, metric)) {
        throw new P1ReadModelError(
          'metric_unavailable',
          `series 未提供登记轨道 ${metric}`,
        )
      }
    }
    return series
  ...
  async readAsns(
    ...
    if (binding.capabilities.affected_as !== 'available') {
      throw new P1ReadModelError(
        'capability_unavailable',
        '当前事件未协商 affected_as=available',
      )
    }
    const payload = await this.get(
      `country-outages/${encodeURIComponent(binding.incident_id)}/asns`,
      {
        publication_id: binding.publication_id,
        page: request.page,
        ...
        sort: request.sort,
        query: request.asn ?? request.query,
      },
      signal,
    )
    assertSameIdentity(payload, binding, 'asns')
    return payload
  }
  ...
  async readPaths(
    ...
    if (binding.capabilities.path_downstreams !== 'available') {
      throw new P1ReadModelError(
        'capability_unavailable',
        '当前事件未协商 path_downstreams=available',
      )
    }
    const payload = await this.get(
      `country-outages/${encodeURIComponent(binding.incident_id)}/path-downstreams`,
      {
        publication_id: binding.publication_id,
        page: request.page,
        page_size: request.pageSize,
        scope: request.scope,
        query: request.query,
        ...(request.affectedAsn === null
          ? {}
          : { affected_asn: request.affectedAsn }),
      },
      signal,
    )
    assertSameIdentity(payload, binding, 'path-downstreams')
    return payload
  }
  ...
  async readAudit(
    ...
    if (binding.capabilities.full_path_evidence !== 'audit_only') {
      throw new P1ReadModelError(
        'capability_unavailable',
        '当前事件未协商 audit_only 证据身份能力',
      )
    }
    const payload = await this.get(
      `country-outages/${encodeURIComponent(binding.incident_id)}/audit`,
      { publication_id: binding.publication_id },
      signal,
    )
    assertSameIdentity(payload, binding, 'audit')
    return payload
  }
