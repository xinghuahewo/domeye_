...
本文件解决“单个任务不能混用版本”；唯一生产主干、不可变制品晋级、发布身份归一
和 Worktree 退役由
[主干开发与发布归一治理规范](主干开发与发布归一治理规范.md)统一约束。任务边界
通过不代表提交已进入生产主干，更不代表已经构建、认证、部署或完成生产验收。
...
普通任务 postflight 通过后只进入评审状态。合入生产主干必须由独立整合任务执行，
并确认：

1. 目标为永久生产主干 `codex/prod`；
2. 源提交已推送且工作树干净；
3. 任务合同、定向检查和人工差异审查均有记录；
4. 合并结果重新执行受影响的集成与发布门禁；
5. 没有把未评审兄弟 Worktree、实验提交或现场文件带入主干；
6. 合并只证明 `merged`，后续仍需不可变构建、认证、部署和生产验收。

任务 Worktree 不因提交合入而自动删除。退役前必须确认其 HEAD 已是
`codex/prod` 的祖先，或已经形成明确的 `excluded` / `abandoned` 决议；同时确认
无未提交文件、提交已推送、证据已保留，并且不被活动发布、回滚、锁、挂载或进程
引用。清理必须逐个精确执行，默认先输出 dry-run 清单。

带发布编号的临时 Worktree 不能长期承担主干职责。当前在途发布使用临时整合
Worktree 的例外在生产验收后终止；以成功发布 tag 建立 `codex/prod`，并将
`core-work` 归一为其永久检出。禁止通过重写历史、强推或复制目录伪造归一结果。
