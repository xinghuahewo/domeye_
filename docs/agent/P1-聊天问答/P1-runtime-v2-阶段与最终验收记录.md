...
国家中断 Agent P1 本地开发候选 `p1-runtime-v2-34dcc7de840e33d8` 已完成
...
| 身份维度 | 冻结值 |
|---|---|
| Candidate | `p1-runtime-v2-34dcc7de840e33d8` |
...
`p1v2_1e703ec0f88d4a79b5bdd128bc128cf9`。
...
2. “伊朗人现在还有互联网吗，是不是全国都断了”：`partial`，2 个执行节点、
   5 条证据。回答 data-through 时点 1,024 个中断前缀，拒绝全国中断与真实用户推断。
...
证据和绑定复验全部通过后提交。桌面 `1440×1000` 与窄屏 `375×812` 均完成真实页面验收。

## 六、工程回归与防伪硬门

- Sidecar 全量：639/639。
- P1 runtime-v2 与 S4 Python 校验器：37/37，其中 S4 篡改负例 12/12。
- 后端同源代理：9/9。
