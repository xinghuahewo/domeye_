...
这里的确定性回退只作为失败闭包中的内部审计制品，不是通过 Response Guard 的
Answer，不向公开接口发布，也不得把本轮标记为 `completed`、
`answer_success=true` 或 `workflow_completed=true`。只有 Renderer 直接生成的草稿
通过 Guard、且最终答案正确时，J1 才记为完成和成功。
...
J2–J5 的“通过”只表示预注册安全断言成立；拒绝、停止、澄清、空结果和 J4 的
确定性回退均不构成工作流完成。
