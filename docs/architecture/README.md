...
5. [用户回答与内部证据边界 v1.0](Domeye_Agent_User_Answer_and_Internal_Evidence_Boundary_v1.0.md)：明确用户正文、内部证据、简洁表达和完成语义；当前状态为 `Designed`。
6. [首个纵向切片回答呈现附加合同 v1.0](Domeye_First_Vertical_Slice_Answer_Presentation_Addendum_v1.0.md)：在不改写锚点 v1.0 的前提下，固定 Answer Context v2、结构化 Draft、Host 拼接和 Guard v2。
7. [M0/M1 目标架构摘要](target-architecture.md)：只看首片直接涉及的 Pi、Host、Tool、Operator、Finding 和回答安全链。
8. [Capability Map](capability-map.md)：按可观察能力查看建设范围和成熟度。
