...
const asnSuggestions = computed(() => {
  ...
  for (const ranking of [
    ...
    overview.value?.volatilityRankings,
...
const rankingSections = computed(() => [
  ...
  },
  {
    ...
    value: (profile: AsnProfile) => profile.resourceChangeRate === null
      ? '—'
  ...
  },
  {
    ...
    rows: overview.value?.volatilityRankings ?? [],
    value: (profile: AsnProfile) => `${profile.volatility.toFixed(1)}%`,
    unit: '',
  },
  {
    key: 'anomaly',
  ...
  <article class="page asn-page">
    <header class="page-heading asn-heading">
      ...
      <p class="page-heading-copy">
        在可审计的运维候选集内比较报文、撤回率、单栈资源变化、波动度和六类异常；该排行不代表全网 ASN 排名。
      </p>
    </header>
      ...
      <section v-if="selected" class="asn-dossier" aria-labelledby="asn-dossier-title">
        ...
        <div class="dossier-metrics">
          ...
          <article>
            ...
            <small>{{ formatDelta(selected.ipv4PrefixChange) }} 较上一窗口</small>
          </article>
          <article>
            <span>IPv6 /48 等效段</span>
            <strong>{{ formatNumber(selected.ipv6Prefixes) }}</strong>
            <small>{{ formatDelta(selected.ipv6PrefixChange) }} 较上一窗口</small>
          </article>
          <article>
            <span>窗口峰值</span>
          ...
          <article>
            <span>波动度</span>
            <strong>{{ selected.volatility.toFixed(1) }}%</strong>
            <small>更新量变异系数</small>
          </article>
          <article>
            <span>异常 / 高风险</span>
        ...
        <div class="asn-chart-grid">
          ...
          <section class="asn-chart-panel">
            <div class="section-heading"><h3>路由资源等效段</h3><span>null ≠ zero</span></div>
