...
async function loadRelated(current: CountryOutageInvestigation, generation: number) {
  ...
  const [graph, receipts] = await Promise.allSettled([graphPromise, receiptPromise])
  if (
...
function submitFollowup() {
  ...
  void mutate(
    () => createCountryOutageInvestigationTurn(current.investigation_id, {
      ...cas('w5-turn'),
    ...
    `追问已绑定 ${anchor.node_id} · REV ${anchor.execution_revision}。`,
  )
}
