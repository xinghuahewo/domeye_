<template>
	<div class="system-menu-container layout-pd">
		<el-dialog v-model="state.notifyDialogVisible" title="通报事件" width="30%">
			<el-form ref="notifyruleFormRef" :model="notifyruleForm" label-width="80px" class="demo-ruleForm" size="default" status-icon>
				<el-form-item label="事件信息" prop="event_info">
					<el-input v-model="notifyruleForm.event_info" type="textarea" :rows="3" disabled />
				</el-form-item>
			</el-form>
			<template #footer>
				<span class="dialog-footer">
					<el-button @click="notifyresetForm(notifyruleFormRef)">取消</el-button>
					<el-button type="primary" @click="notifysubmitForm(notifyruleFormRef)">确定</el-button>
				</span>
			</template>
		</el-dialog>
...
<!--        <el-input size="default" v-model="state.event_info" placeholder="前缀、AS名称、国家" style="max-width: 150px" class="ml10"> </el-input>-->
          ...
          <el-table-column prop="attacked_org" label="受害方机构" min-width="85" sortable="custom" />
          ...
          <el-table-column prop="attacker_org" label="肇事方机构" min-width="85" sortable="custom" />
...
<!--							<el-button type="primary" link @click="report(scope.row.detail_url)" class="ml10">报告</el-button>-->
								...
								<el-button v-if="userInfo.roles[0] !== 'guest'" type="primary" link @click="handleNotify(scope.row)" class="ml10">通报</el-button>
							</div>
						</template>
					</el-table-column>
				</el-table>
...
// };
// 通报
const notifyruleFormRef = ref<FormInstance>();
const notifyruleForm = reactive({
	detail_url: '',
	event_info: '',
});
const notifysubmitForm = async (formEl: FormInstance | undefined) => {
	if (!formEl) return;
	await formEl.validate((valid, fields) => {
		if (valid) {
			...
			state.notifyDialogVisible = false;
		} else {
			console.log('error submit!', fields);
		}
	});
};
const notifyresetForm = (formEl: FormInstance | undefined) => {
	if (!formEl) return;
	formEl.resetFields();
	state.notifyDialogVisible = false;
};
const notify = async () => {
	const notifyResult = await request({
    // url: 'http://10.3.242.226:19746/notify',
		url: baseUrl + 'events/notify',
		method: 'post',
		data: {
			detail_url: notifyruleForm.detail_url,
		},
	});
	if (notifyResult.status) {
		ElMessage.success('事件通报成功');
		getTableData();
	} else {
		const msg = notifyResult.msg ? notifyResult.msg : '事件通报失败';
		ElMessage.error(msg);
	}
};
const handleNotify = (row) => {
	notifyruleForm.detail_url = row.detail_url;
	notifyruleForm.event_info = row.event_info;
	state.notifyDialogVisible = true;
};
// 页面加载时
onMounted(() => {
	getTableData();
