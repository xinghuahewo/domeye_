  ...
  <div class="system-menu-container layout-pd">
    <el-card shadow="hover">
      <div class="system-menu-search mb15" style="display: flex">
        <el-select size="default" v-model="state.level" placeholder="事件等级" style="max-width: 150px">
          <el-option v-for="item in levels" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
        <!-- 更改：机构 -> 肇事方和受害方 -->
        <el-input size="default" v-model="state.attacker_org" placeholder="肇事方" style="max-width: 90px" class="ml10"> </el-input>
        <el-input size="default" v-model="state.attacked_org" placeholder="受害方" style="max-width: 90px" class="ml10"> </el-input>
...
<!--        <el-input size="default" v-model="state.event_info" placeholder="前缀、AS名称、国家" style="max-width: 150px" class="ml10"> </el-input>-->
      ...
      <div style="display: flex;position: relative">
        <el-date-picker
            ...
            style="max-width: 280px"
        />
        <el-input size="default" v-model="state.notify_username" placeholder="通报人" style="max-width: 150px" class="ml10"> </el-input>
        <el-date-picker
            v-model="state.notify_time"
            type="daterange"
            range-separator="-"
            start-placeholder="通报时间起点"
            end-placeholder="通报时间终点"
            class="ml10"
            size="default"
            style="max-width: 280px"
        />
      ...
      <div>
        ...
        >
          ...
          <el-table-column prop="event_type" label="事件类型" width="100" sortable="custom" />
          <el-table-column prop="affected_prefix" label="影响前缀" min-width="130" />
          <el-table-column prop="attacked_as" label="受害方AS" min-width="85" sortable="custom">
            <template #default="scope">
              <span>{{ scope.row.attacked_as }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="attacked_org" label="受害方机构" min-width="90" sortable="custom" />
          <el-table-column prop="attacked_country" label="受害方国家" width="120" sortable="custom" />
          <el-table-column prop="attacker_as" label="肇事方AS" min-width="85" sortable="custom" />
          <el-table-column prop="attacker_org" label="肇事方机构" min-width="90" sortable="custom" />
          <el-table-column prop="attacker_country" label="肇事方国家" width="120" sortable="custom" />
          <el-table-column prop="level" label="事件等级" width="100" sortable="custom">
            <template #default="scope">
          ...
          <el-table-column prop="start_time" width="100" label="开始时间" sortable="custom" />
          <el-table-column prop="end_time" width="100" label="结束时间" sortable="custom" />
          <el-table-column prop="judge_reason" width="95" label="研判依据" sortable="custom" />
          <el-table-column prop="judge_username" width="80" label="研判人" sortable="custom" />
          <el-table-column prop="judge_time" width="95" label="研判时间" sortable="custom" />
          <el-table-column prop="notify_username" width="80" label="通报人" sortable="custom" />
          <el-table-column prop="notify_time" width="100" label="通报时间" sortable="custom" />
          <el-table-column label="操作" width="100">
            <template #default="scope">
              <el-button type="primary" link @click="toDetail(scope.row.detail_url, scope.row.event_type)">详情</el-button>
              <el-button type="primary" link @click="report(scope.row.detail_url)" class="ml10">报告</el-button>
            </template>
          </el-table-column>
        </el-table>
  ...
  try {
    const res = await request({
      ...
      params: {
        ...
        notify_username: state.notify_username,
        notify_time: state.notify_time === '' ? '' : dateToString(state.notify_time[0]) + '_' + dateToString(state.notify_time[1]),
      },
    });
...
// 导出报告
const report = async (detail_url) => {
  const download_url = await request({
    // url: 'http://10.3.242.226:19746/word-export',
