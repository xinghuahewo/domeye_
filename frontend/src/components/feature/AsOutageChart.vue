...
const initChart = () => {
  if (chartContainer.value && !chartInstance) {
    ...
    console.log('AS中断图表实例初始化成功');
  }
};

// 更新图表
const updateChart = (data: AsOutageData[], title: string) => {
  if (!chartInstance) {
  ...
  try {
    ...
    console.error('AS中断图表更新失败:', error);
  }
};
