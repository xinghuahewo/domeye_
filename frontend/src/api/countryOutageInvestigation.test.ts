...
describe('W5 组合调查 API', () => {
  ...
  it('按调查、Turn 与 revision 读取版本化回答', async () => {
    await getCountryOutageInvestigationTurn('inv/1', 'turn/1', 3)
    expect(fetchMock.mock.calls.at(-1)?.[0]).toBe(
      '/api/v2/country-outage/investigations/inv%2F1/turns/turn%2F1/revisions/3',
    )
  })
