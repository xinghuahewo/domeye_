...
export function getCountryOutageInvestigationTurn(
  investigationId: string,
  turnId: string,
  turnRevision: number,
) {
  return requestJson<CountryOutageInvestigationTurnEnvelope>(
    `country-outage/investigations/${safe(investigationId)}/turns/${safe(turnId)}/revisions/${turnRevision}`,
  )
}
...
export function createCountryOutageInvestigationTurn(
  ...
  return post<CountryOutageInvestigationTurnActionResponse>(
...
export function getCountryOutageInvestigationReceipts(
  investigationId: string,
  kind?: 'tool' | 'operator' | 'model' | 'gate' | 'cost' | 'latency' | 'reuse' | 'transaction',
) {
