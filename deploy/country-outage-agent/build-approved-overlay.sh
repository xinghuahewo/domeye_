...
copy_approved_payload() {
    ...
    # 风险分级、受影响测试、发布边界测试和只读生产库存采集用于组合归档后的逐文件闭环。
    for relative in \
        dev/checks.py \
        dev/tests/test_checks.py \
        dev/tests/test_country_outage_agent_release_boundary.py \
    ...
    # 前端只带本任务明确修改/新增的源码与测试，支持组合后 Git archive 逐文件闭环。
    # 严禁携带 dist：组合后必须从已绑定的完整源码基线和固定数据档重新构建统一前端。
    for relative in \
        ...
        frontend/src/pages/EventsPage.vue \
...
validate_payload_boundary() {
    ...
    forbidden_pattern='/(backend/core|node_modules|__pycache__|\.pytest_cache|\.cache|coverage|frontend/dist|frontend/tmp|output|tmp|artifacts)(/|$)'
...
4. 从组合后的完整源码树读取 `config/data-profile.json`，重新构建统一前端；
5. 对组合后的完整树重新生成 manifest 和全文件 SHA256SUMS；
6. 重新执行核心回归、Sidecar readiness、Python 控制面联合健康检查、
   同一认证 profile 的一次真实核心报告冒烟；
7. 先 canary，保留基础归档与前端树回滚点，再允许原子切换。
...
- 本任务获批的前端源码与测试；不包含任何预构建 `frontend/dist`；
- 只读生产库存采集脚本、中文说明和测试。
...
- `node_modules`、Sidecar 编译输出、测试输出、缓存、coverage、
  `frontend/dist`、`frontend/tmp`、sourcemap、AppleDouble 和 `.DS_Store`；
...
write_metadata() {
    ...
    local payload_digest core_manifest_digest
    ...
    jq -n \
        ...
        '{
          ...
          boundaries: {
            ...
            frontend_dist_included: false,
            frontend_activation_authorized: false,
            frontend_build_required: true,
            frontend_source_mode: "approved_sources_only_on_bound_complete_base",
