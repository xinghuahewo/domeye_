...
| 文件 | 职责 |
| --- | --- |
| `pre-receive` | 保护 `codex/prod` 和正式发布时间戳 tag |
...
`codex/prod` 和正式 tag 执行失败关闭：

- `codex/prod` 只能快进，禁止删除和强推；
- 新目标提交必须具有格式正确的评审与 CI 证明；
- 已存在 tag 禁止修改和删除；
- 名称匹配 `YYYYMMDDTHHMMSSZ-*` 的 tag 必须为 annotated tag；
- 正式 tag 目标必须已经进入当前或同批更新的 `codex/prod`。
...
- `/home/bgpdata/Domeye-Core` 中的 `codex/prod` 与 annotated tag；
