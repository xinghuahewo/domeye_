...
4. 断言隔离 Node.js 为 `v22.23.1`，再执行 `npm ci`、前端测试和生产构建。
...
候选验收不会修改生产信息目录、Nginx 或活动数据库链接。生产切换前会从实际 Screen 进程树安全捕获其 `INFO_DIR`，不读取或打印其他环境密钥。切换期间如完整冒烟失败，脚本会消费一次性回滚日志，恢复原信息目录、`.env`、活动链接、Nginx 和原运行状态；首次安装前不存在 `.env`、信息目录或活动数据库链接时，也会恢复为“不存在”而不是制造占位状态。
