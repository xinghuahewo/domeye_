...
- 治理迁移完成后，永久生产主干固定为 `codex/prod`，本地永久检出固定为
...
- 生产发布只接受 `codex/prod` 上经过统一发布回归的 annotated tag，以及由该
  提交一次构建并按摘要冻结的不可变制品。候选、金丝雀和生产必须晋级同一制品。
- `implemented`、`committed`、`reviewed`、`merged`、`built`、`certified`、
  `staged`、`deployed` 和 `verified` 是不同状态，不得用 Hook、HTTP 200、Screen、
  tag 或可见页面越级宣告生产完成。
- 当前在途发布可以继续使用已冻结的临时整合 Worktree 收尾；该例外必须在任务合同
  中绑定明确提交，并在生产验收后以成功 tag 建立 `codex/prod`。不得把带历史
  `prodNN` 名称的临时分支继续用作下一轮永久主干。
- 发布归一必须证明 `core-work HEAD`、`origin/codex/prod`、release tag、源码归档
  和生产运行提交一致，并分别核对 Backend、Sidecar、Frontend、Nginx 与数据库
  身份。任一漂移时不得写成 `verified`。
- 已整合的任务 Worktree 只有在干净、已推送、证据已保留且不被发布或运行时引用时
  才能逐个退役；未整合研究、文档或实验分支不得因清理 Worktree 而自动合并。
...
- 普通任务完成后只能提交评审，不得自行合入 `codex/prod`。跨分支整合、发布归一
  和紧急修复必须使用独立任务合同，并把源提交、目标提交、制品身份和回滚边界写入
  合同。
