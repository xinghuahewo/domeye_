...
""",
    ...
    def test_w6_guard_resealed_task_identity_is_exact_and_unknown_task_is_rejected(self) -> None:
        self.assertIn(
            "w6_certifier_only_task_boundary_verified",
            HOOK.validate_task(self.root),
        )
        self.assertTrue(HOOK._w6_task_active(self.root))
        evidence = self.create_w0_fixture()
        reference = next(
            item for item in evidence["test_receipts"]
            if item["suite_id"] == "w0-python"
        )
        HOOK._validate_stage_test_run_receipt(self.root, reference, "w0-python")
        task = self.load(HOOK.TASK_PATH)
        task["taskId"] = "country-outage-agent-p2-s1-w6-offline-certification-v4-forged"
        self.write(HOOK.TASK_PATH, task)
        self.assert_alignment_error(
            "task_identity_mismatch",
            lambda: HOOK.validate_task(self.root),
        )
