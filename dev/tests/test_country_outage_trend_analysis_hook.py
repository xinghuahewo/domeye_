...
HOOK_PATH = (
    ...
    / ".codex"
    / "hooks"
    / "country_outage_trend_analysis_review.py"
)


def load_hook_module():
    specification = importlib.util.spec_from_file_location(
        "country_outage_trend_analysis_review",
        HOOK_PATH,
    )
...
class CountryOutageTrendAnalysisHookTest(unittest.TestCase):
    def run_hook(
        self,
        *arguments: str,
    ...
    def test_every_explicit_stage_emits_review_without_claiming_acceptance(self) -> None:
        for stage in (f"S{index}" for index in range(7)):
            with self.subTest(stage=stage):
                ...
                self.assertIn("不代表数据、算法、API、页面", result.stdout)
                self.assertIn(
                    f"国家中断趋势分析最终验收回检：{stage}",
                    result.stdout,
                )
    ...
    def test_stop_hook_requests_review_for_declared_stage(self) -> None:
        result = self.run_hook(
            input_text=json.dumps({"hook_event_name": "Stop"}),
            environment={"DOMEYE_COUNTRY_OUTAGE_TREND_STAGE": "S3"},
        )
        self.assertEqual(result.returncode, 0, result.stderr)
        payload = json.loads(result.stdout)
        self.assertEqual(payload.get("decision"), "block")
        reason = payload.get("reason", "")
        self.assertIn("TAE-08、TAE-09、TAE-10", reason)
    ...
    def test_stop_hook_avoids_recursion(self) -> None:
        result = self.run_hook(
            ...
            environment={"DOMEYE_COUNTRY_OUTAGE_TREND_STAGE": "S2"},
        )
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(json.loads(result.stdout), {})
