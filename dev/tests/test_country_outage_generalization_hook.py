...
class CountryOutageGeneralizationHookTest(unittest.TestCase):
    ...
    def test_s2_stage_verifier_is_registered(self) -> None:
        module = load_hook_module()
        verifier = module.STAGE_VERIFIER_PATHS.get("S2")
        self.assertEqual(
            verifier,
            REPOSITORY_ROOT / "dev" / "verify_country_outage_generalization_s2.py",
        )

    def test_s3_stage_verifier_is_registered(self) -> None:
        module = load_hook_module()
        verifier = module.STAGE_VERIFIER_PATHS.get("S3")
        self.assertEqual(
            verifier,
    ...
    def test_s5_stage_verifier_is_registered(self) -> None:
        module = load_hook_module()
        verifier = module.STAGE_VERIFIER_PATHS.get("S5")
        self.assertEqual(
            verifier,
            REPOSITORY_ROOT / "dev" / "verify_country_outage_generalization_s5.py",
        )

    def test_s6_stage_verifier_is_registered(self) -> None:
        module = load_hook_module()
        verifier = module.STAGE_VERIFIER_PATHS.get("S6")
        self.assertEqual(
            verifier,
            REPOSITORY_ROOT / "dev" / "verify_country_outage_generalization_s6.py",
        )
