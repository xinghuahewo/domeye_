...
def validate_live_evidence(
    ...
    for case_id, contract in p0_by_id.items():
        ...
        for index, (turn, (required, allowed_goals)) in enumerate(
            ...
            if case_id == "P013-B-05":
                result_by_kind = {
                    item.get("normalized_kind"): item
                    for item in results
                    if isinstance(item, dict)
                }
                expected_boundary_text = {
                    "cause_or_responsibility": ("责任主体",),
                    "real_user_or_national_impact": ("受影响用户数",),
                    "economic_impact": ("经济损失", "业务损失", "经济证据"),
                }
                for kind, fragments in expected_boundary_text.items():
                    result = result_by_kind.get(kind)
                    text = result.get("text") if isinstance(result, dict) else None
                    if not isinstance(text, str) or any(
                        fragment not in text for fragment in fragments
                    ):
                        errors.append(f"{case_id} {kind} 缺少独立边界解释")
                economic = result_by_kind.get("economic_impact")
                if isinstance(economic, dict) and "责任主体" in str(economic.get("text")):
                    errors.append(f"{case_id} economic_impact 错误复用责任边界")
                if (
                    execution.get("nodes") != []
                    or evidence != []
                    or receipt.get("status") != "none"
                ):
                    errors.append(f"{case_id} 三个越界目标必须零执行、零证据、状态不提交")
                goals_by_id = {
                    item.get("goal_id"): item.get("normalized_kind")
                    for item in goals
                    if isinstance(item, dict)
                }
                reason_by_kind = {
                    goals_by_id.get(item.get("goal_id")): item.get("reason_codes")
                    for item in grounding.get("decisions", [])
                    if isinstance(item, dict)
                }
                expected_reasons = {
                    "cause_or_responsibility": [
                    ...
                    "economic_impact": ["rrc25_has_no_economic_impact_evidence"],
                }
                if reason_by_kind != expected_reasons:
                    errors.append(f"{case_id} 三个越界目标的政策理由未逐目标闭合")
