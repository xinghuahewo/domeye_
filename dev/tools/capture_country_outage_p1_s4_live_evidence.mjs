...
const p0 = await captureP0()
const semantic = await captureSemanticVariants()
process.stdout.write(`${JSON.stringify({ p0: p0.counts, semantic: semantic.counts })}\n`)
if (p0.counts.failed > 0 || semantic.counts.failed > 0) process.exitCode = 1
